...
impl PictureData {
	...
	pub fn new_from_slice(slice: &Slice, sps: &Sps, timestamp: u64, first_field: Option<&RcPictureData>) -> Self {
		...
		let (pic_order_cnt_lsb, delta_pic_order_cnt_bottom, delta_pic_order_cnt0, delta_pic_order_cnt1) =
	...
	pub fn is_second_field_of_complementary_ref_pair(&self) -> bool {
		self.is_ref() && matches!(self.field_rank(), FieldRank::Second(first_field) if first_field.borrow().is_ref())
	}
