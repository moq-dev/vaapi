...
impl TryFrom<bindings::VAGenericValue> for GenericValue {
	...
	fn try_from(value: bindings::VAGenericValue) -> Result<Self, Self::Error> {
		...
		match value.type_ {
			// Safe because we check the type before accessing the union.
			bindings::VAGenericValueType::VAGenericValueTypeInteger => Ok(Self::Integer(unsafe { value.value.i })),
			bindings::VAGenericValueType::VAGenericValueTypeFloat => Ok(Self::Float(unsafe { value.value.f })),
			bindings::VAGenericValueType::VAGenericValueTypePointer => Ok(Self::Pointer(unsafe { value.value.p })),
			bindings::VAGenericValueType::VAGenericValueTypeFunc => Ok(Self::Func(unsafe { value.value.fn_ })),
